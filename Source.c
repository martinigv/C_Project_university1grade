...
void createMonster()
{
    ...
    {
    case Slime:
        printf("슬라임 스폰중 (HP: 15 / ATT: 5 / DEF: 0)\n");
        monsterHp = 15;
        monsterAttack = 5;
        monsterDefence = 0;
        monsterExp = 10;
        if (playerLevel >= 5)
            break;
    case Orce:
        printf("오크 스폰중 (HP: 50 / ATT: 15 / DEF: 15)\n");
        monsterHp = 50;
        monsterAttack = 15;
        monsterDefence = 8;
        monsterExp = 50;
        ...
        break;
    case Troll:
        printf("트롤 스폰중 (HP: 30 / ATT: 10 / DEF: 5)\n");
        monsterHp = 30;
        monsterAttack = 10;
        monsterDefence = 5;
        monsterExp = 30;
...
void enterBattle()
{
    while (1)
    {
        ...
        int playerDamaged = monsterAttack - plyaerDefence; // 플레이어가 입는 데미지(몬스터 턴)
        if (playerDamaged <= 0)
            playerDamaged = 1;
...
void upgradestat()
{
    for (int i = 1; playerLevel >= i * 5; i++) // 5, 10, 15 ... 100이 될 때마다 확인하는 것보단 반복문을 쓰는게 더 낫다고 판단했다.
    {
        playerAttack += i * 5;               // 플레이어 스탯 상승(레벨이 5, 10, 15 ...5의 배수가 될 때)
        playerAttack -= (i - 1) * i / 2 * 5; // 이 값을 빼주는 이유는 15레벨일 때 15를 올려주는게 목적인데, 
        // 반복문 때문에 5(1+2+3)을 올려주게 되므로 5(1+2)의 값을 빼주는 것이다. 
        playerHp += i * 30;
        playerHp -= (i - 1) * i / 2 * 30;
        plyaerDefence += i * 2;
        plyaerDefence -= (i - 1) * i / 2 * 2;

        monsterAttack += i * 3; // 몬스터들도 강화됨
